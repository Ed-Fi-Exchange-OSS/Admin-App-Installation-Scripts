...
<#
.SYNOPSIS
Tests the shared SQL-target helpers in edorg-sync\compat.ps1.

.DESCRIPTION
Covers the decisions that switch the Education Organization sync scripts between a
local SQL Server instance and a remote target such as a managed Azure SQL Database:
remote-target detection across the host formats sqlcmd accepts, the sqlcmd
encryption/certificate flags per target, that those flags survive splatting to a
native command, and the refusal of Windows integrated authentication against a
remote target. These run without a database, a server, or any module: plain
PowerShell, so they work on both Windows PowerShell 5.1 and PowerShell 7.

Mirrors quick-start\tests\Test-SqlCompat.ps1 and windows-install\tests\Test-SqlCompat.ps1,
which cover the same helpers for their own folders. The copies must agree: an
operator who points the installer, the Quick Start, and this sync at the same server
expects all three to secure the connection the same way.
...
Assert-Throws -Because 'integrated security is refused for a remote host, naming the target and the way out' `
    -Matching 'myserver.database.windows.net', '-UseIntegratedSecurity', '-DbUsername', '-DbPassword', 'SQL authentication' -Action {
    Assert-SqlAuthSupported -SqlServer 'myserver.database.windows.net' -UseIntegratedSecurity $true -UsernameParameterName '-DbUsername' -PasswordParameterName '-DbPassword'
}
Assert-DoesNotThrow -Because 'integrated security is allowed for a local instance' -Action {
    Assert-SqlAuthSupported -SqlServer 'tcp:localhost,1433' -UseIntegratedSecurity $true -UsernameParameterName '-DbUsername' -PasswordParameterName '-DbPassword'
}
Assert-DoesNotThrow -Because 'a remote host with SQL authentication is allowed' -Action {
    Assert-SqlAuthSupported -SqlServer 'myserver.database.windows.net' -UseIntegratedSecurity $false -UsernameParameterName '-DbUsername' -PasswordParameterName '-DbPassword'
}
...
$otherFolder = & {
    . "$PSScriptRoot/../../quick-start/compat.ps1"
    foreach ($h in $hostMatrix) {
        '{0}|{1}|{2}' -f $h, ((Get-SqlcmdTrustArgs -SqlServer $h) -join ' '), ((Get-SqlcmdTrustArgs -SqlServer $h -TrustServerCertificate) -join ' ')
    }
}
Assert-Equal -Expected ($thisFolder -join "`n") -Actual ($otherFolder -join "`n") -Because "quick-start/compat.ps1 returns the same flags for all $($hostMatrix.Count) targets"

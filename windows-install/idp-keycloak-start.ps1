...
.DESCRIPTION
Sets KC_BOOTSTRAP_ADMIN_USERNAME and KC_BOOTSTRAP_ADMIN_PASSWORD environment variables for
...
If Keycloak was previously bootstrapped with a different admin password, the environment
...
# its type conversion and fail) for the KC_BOOTSTRAP_ADMIN_* environment variables below.
...
# 3. Start Keycloak in the background. The bootstrap environment variables are set on THIS
...
# The child already has its own copy of the environment block; don't leave the admin
...
Write-Host "Started Keycloak (process ID $($proc.Id)). Startup log: $startupLog"
